...
names(dat_all_inr)
vars <- names(dat_all_inr)[c(2,5:8,30:198)]
...
d_pov_highcog <- dat_all_inr %>% filter(welloff_highflcog==1) %>% 
  mutate(profile="welloff_highflcog",
         resilience=1)
d_pov_lowcog <- dat_all_inr %>% filter(welloff_lowflcog==1) %>% 
...
names(d_flpov)
new_tr <- as.matrix(d_flpov[,c(5,30:198)])
...
write.csv(table_util_imp_label_gain,"/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/abcd_study_with_kat/results/INR_quart_WELLOFF_fluid_cvXgb_table_util_Missing_label_gain.csv")


#-- Run a logistic regression with the top 20 important predictors selected by xgboost
glm_cvxgboost_completecase = glm(data=d_flpov, resilience ~ reshist_addr1_coi_se_public+birth_weight+demo_prtnr_ed_v2+
                                   asr_scr_anxdep_t+sds_p_ss_sbd+reshist_addr1_coi_se_home+reshist_addr1_svi_tot_20142018+
                                   reshist_addr1_coi_se_emprat+devhx_18_p+physical_activity1_y+pmq_y_ss_mean+
                                   reshist_addr1_coi_he_walk+cbcl_scr_syn_social_r+reshist_addr1_coi_he_pm25+asr_scr_attention_t+
                                   reshist_addr1_popdensity+reshist_addr1_coi_ed_prxece+reshist_addr1_coi_ed_hsgrad+reshist_addr1_coi_ed_teachxp+
                                   reshist_addr1_coi_se_occ, family="binomial")

summary(glm_cvxgboost_completecase)

pred <- predict(glm_cvxgboost_completecase, newdata=d_flpov, type="response")
...
d_pov_highflcog <- dat_all_inr_complete %>% filter(welloff_highflcog==1) %>% 
  mutate(profile="welloff_highflcog",
         resilience=1)
d_pov_lowflcog <- dat_all_inr_complete %>% filter(welloff_lowflcog==1) %>% 
  mutate(profile="welloff_lowflcog",
         resilience=0)
...
write.csv(table_util_imp_label_gain,"/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/abcd_study_with_kat/results/INR_quart_WELLOFF_fluid_cvXgb_complete_cases_table_util_imp_label_gain.csv")
...
d_pov_highflcog <- d %>% filter(welloff_highflcog==1) %>% 
  mutate(profile="welloff_highflcog",
         resilience=1)
d_pov_lowflcog <- d %>% filter(welloff_lowflcog==1) %>% 
  mutate(profile="welloff_lowflcog",
         resilience=0)
...
write.csv(table_util_imp_label_gain,"/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/abcd_study_with_kat/results/INR_quart_WELLOFF_fluid_cvXgb_imputed1x_table_util_imp_label_gain.csv")
...
write.csv(table_util_imp_label_gain,"/Users/Kat/Library/CloudStorage/OneDrive-HarvardUniversity/abcd_study_with_kat/results/INR_quart_WELLOFF_fluid_cvSL_imputed1x_table_util_imp_label_gain.csv")
