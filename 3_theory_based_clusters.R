...
d_pov <- rbind(d_pov_highcog,d_pov_lowcog)

#-- Look at distributions
names(d_pov)

vars <- names(d_pov)[c(2:7)]
...
names(table_by_pov) <- c("Characteristic", "Overall", "Poverty_High_Cog", "Poverty_Low_Cog", "p_value")
table_by_pov
...
table_by_pov <- data.frame(table_by_pov) %>%
  ...
  dplyr::select(rownames,Poverty_High_Cog,Poverty_Low_Cog,Overall,p_value)
